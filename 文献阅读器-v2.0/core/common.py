...
def http_get_text(url: str, *, timeout: int = 30, headers: dict[str, str] | None = None) -> str:
    """Get text from URL with proper error handling."""
    try:
        ...
        raise Exception(f"HTTP error {e.code} when accessing {url}: {e.reason}")
    except Exception as e:
        raise Exception(f"Error fetching {url}: {e}")


def http_get_json(url: str, *, timeout: int = 30, headers: dict[str, str] | None = None) -> dict[str, Any]:
    """Get JSON from URL with proper error handling."""
    try:
        content = http_get_text(url, timeout=timeout, headers=headers)
        return json.loads(content)
    except json.JSONDecodeError as e:
        raise Exception(f"Invalid JSON response from {url}: {e}")
    except Exception as e:
        raise Exception(f"Error fetching JSON from {url}: {e}")


def http_get_bytes(url: str, *, timeout: int = 60, headers: dict[str, str] | None = None) -> bytes:
    """Get bytes from URL with proper error handling."""
    try:
        request = urllib.request.Request(url, headers=headers or {"User-Agent": DEFAULT_USER_AGENT})
        with urllib.request.urlopen(request, timeout=timeout) as response:
...
def batch_http_get(urls: list[str], *, timeout: int = 30, headers: dict[str, str] | None = None, max_workers: int = 5) -> list[tuple[str, str | None]]:
    """Batch fetch multiple URLs concurrently.
    
    Returns a list of tuples (url, content), where content is None if the request failed.
    """
...
def batch_http_get_json(urls: list[str], *, timeout: int = 30, headers: dict[str, str] | None = None, max_workers: int = 5) -> list[tuple[str, dict[str, Any] | None]]:
    """Batch fetch and parse multiple JSON URLs concurrently.
    
    Returns a list of tuples (url, data), where data is None if the request or parsing failed.
    """
    results: list[tuple[str, dict[str, Any] | None]] = []
    
    def fetch_json(url: str) -> tuple[str, dict[str, Any] | None]:
        try:
            content = http_get_text(url, timeout=timeout, headers=headers)
...
def resolve_reference(value: str) -> dict[str, Any]:
    """Resolve a paper reference to a structured record with proper error handling."""
    try:
        ...
        if source_type == "local_pdf":
            try:
                ...
                if not path.exists():
                    raise Exception(f"Local PDF file not found: {path}")
                if not path.is_file():
                    raise Exception(f"Path is not a file: {path}")
                if path.suffix.lower() != ".pdf":
                    raise Exception(f"File is not a PDF: {path}")
                ...
                raise Exception(f"Error resolving local PDF: {e}")
        ...
        if source_type == "arxiv_id":
            try:
                ...
                if papers:
                    ...
                    paper["status"] = "ok"
                    return paper
                raise Exception(f"No arXiv record found for ID: {arxiv_id}")
            except Exception as e:
                raise Exception(f"Error resolving arXiv ID: {e}")
        
        if source_type == "arxiv_url":
            try:
                arxiv_id = extract_arxiv_id(stripped)
                if not arxiv_id:
                    raise Exception("Invalid arXiv URL")
                papers = safe_fetch_arxiv_entries(id_list=arxiv_id, max_results=1)
                if papers:
                    paper = papers[0]
                    paper["paper_id"] = paper_id_for_record(paper)
                    paper["status"] = "ok"
                    return paper
                raise Exception(f"No arXiv record found for URL: {stripped}")
            except Exception as e:
                raise Exception(f"Error resolving arXiv URL: {e}")
        ...
        if source_type in {"doi", "doi_url"}:
            try:
                ...
                raise Exception(f"Error resolving DOI: {e}")
        ...
        if source_type == "pdf_url":
            try:
                ...
                raise Exception(f"Error resolving PDF URL: {e}")
        ...
        if source_type == "zotero_key":
            try:
                ...
                raise Exception(f"Error resolving Zotero key: {e}")
        ...
        try:
            ...
            raise Exception(f"Error resolving title query: {e}")
    except Exception as e:
        raise Exception(f"Error resolving reference '{value}': {e}")
...
def enrich_metadata(record: dict[str, Any]) -> dict[str, Any]:
    ...
    if title:
        ...
        if title:
            # 构建搜索URLs
            semantic_scholar_url = f"{SEMANTIC_SCHOLAR_SEARCH_URL}?query={urllib.parse.quote(title)}&limit=5&fields=title,abstract,year,venue,url,externalIds,authors.name,authors.affiliations"
            search_urls.append(semantic_scholar_url)
            
            openalex_url = f"{OPENALEX_WORKS_URL}?search={urllib.parse.quote(title)}&per-page=5"
            search_urls.append(openalex_url)
            
            crossref_url = f"{CROSSREF_WORKS_URL}?query.title={urllib.parse.quote(title)}&rows=5"
            search_urls.append(crossref_url)
        
        # 批量获取搜索结果
        if search_urls:
            results = batch_http_get_json(search_urls, max_workers=3)
            
            # 处理搜索结果
            for url, data in results:
                if data:
                    if SEMANTIC_SCHOLAR_SEARCH_URL in url:
                        # 处理Semantic Scholar结果
                        items = data.get("data", []) or []
                        sem_papers = [normalize_semantic_scholar_paper(item) for item in items if isinstance(item, dict)]
                        sem = choose_best_title_match(title, sem_papers)
                        if sem:
                            candidates.append(sem)
                    elif OPENALEX_WORKS_URL in url:
                        # 处理OpenAlex结果
                        items = data.get("results", []) or []
                        oa_papers = [normalize_openalex_work(item) for item in items if isinstance(item, dict)]
                        oa = choose_best_title_match(title, oa_papers)
                        if oa:
                            candidates.append(oa)
                    elif CROSSREF_WORKS_URL in url:
                        # 处理CrossRef结果
                        items = data.get("message", {}).get("items", []) or []
                        cross_papers = [normalize_crossref_work(item) for item in items if isinstance(item, dict)]
                        cross = choose_best_title_match(title, cross_papers)
                        if cross:
...
def load_config_file() -> dict[str, Any]:
    ...
    config_paths = [
        Path.cwd() / "deep_paper_note_config.json",
        Path.home() / ".deep_paper_note_config.json",
        Path.home() / ".config" / "deep_paper_note" / "config.json",
    ]
...
def runtime_config() -> dict[str, Any]:
    """Get runtime configuration from config file and environment variables."""
    # Load from config file first
    config_file = load_config_file()
    
    # Override with environment variables
    return {
        "obsidian_vault": config_file.get("obsidian_vault") or env_config_value(
            "DEEPPAPERNOTE_OBSIDIAN_VAULT",
            "READ_ARXIV_OBSIDIAN_VAULT",
        ),
        "papers_dir": config_file.get("papers_dir") or env_config_value("DEEPPAPERNOTE_PAPERS_DIR", default="Research/Papers"),
        "output_dir": config_file.get("output_dir") or env_config_value("DEEPPAPERNOTE_OUTPUT_DIR", default="tmp/DeepPaperNote"),
        "workspace_output_dir": config_file.get("workspace_output_dir") or env_config_value(
            "DEEPPAPERNOTE_WORKSPACE_OUTPUT_DIR",
            default="DeepPaperNote_output",
        ),
        "cache_dir": config_file.get("cache_dir") or env_config_value(
            "DEEPPAPERNOTE_CACHE_DIR",
            default="tmp/DeepPaperNote_cache",
        ),
        "cache_enabled": config_file.get("cache_enabled") if "cache_enabled" in config_file else (
            env_config_value("DEEPPAPERNOTE_CACHE_ENABLED", default="true").lower() in {"true", "1", "yes", "on"}
        ),
        "cache_ttl": config_file.get("cache_ttl") or int(env_config_value(
            "DEEPPAPERNOTE_CACHE_TTL",
            default="86400",  # 24 hours in seconds
        )),
        "max_workers": config_file.get("max_workers") or 5,
        "timeout": config_file.get("timeout") or 30,
        "max_pages": config_file.get("max_pages") or 24,
        "min_searchable_chars": config_file.get("min_searchable_chars") or 100,
    }
...
def cache_dir(config: dict[str, Any] | None = None) -> Path:
    ...
    cache_dir_path = Path(config.get("cache_dir", "tmp/DeepPaperNote_cache")).expanduser().resolve()
    cache_dir_path.mkdir(parents=True, exist_ok=True)
    return cache_dir_path


def cache_file_path(paper_id: str, suffix: str) -> Path:
    """Get the cache file path for a given paper and suffix."""
    config = runtime_config()
    if not config.get("cache_enabled", True):
        return Path()
    return cache_dir(config) / f"{paper_id.replace(':', '_')}_{suffix}.json"


def load_from_cache(paper_id: str, suffix: str) -> dict[str, Any] | None:
    """Load data from cache if available and not expired."""
    config = runtime_config()
    if not config.get("cache_enabled", True):
        return None
    
    cache_path = cache_file_path(paper_id, suffix)
    ...
    try:
        ...
        if current_time - stat.st_mtime > config.get("cache_ttl", 86400):
            return None
...
def save_to_cache(paper_id: str, suffix: str, data: dict[str, Any]) -> None:
    """Save data to cache."""
    config = runtime_config()
    if not config.get("cache_enabled", True):
        return
...
def clear_cache() -> None:
    ...
    try:
        cache_dir_path = cache_dir(config)
...
DOMAIN_RULES: list[tuple[str, tuple[str, ...]]] = [
    ("心理健康", ("mental health", "depression", "anxiety", "psychiatric", "psychology", "clinical", "patient", "counsel", "therapy", "心理", "抑郁", "焦虑", "精神", "临床", "患者", "咨询", "治疗")),
    ("大模型", ("large language model", "llm", "foundation model", "gpt", "transformer", "instruction tuning", "pretrain", "pre-training", "language model", "agent", "reasoning", "大模型", "语言模型", "基础模型", "指令微调", "预训练", "智能体", "推理")),
    ("多模态", ("multimodal", "vision-language", "audio-visual", "video-language", "image-text", "cross-modal", "多模态", "视觉语言", "视听", "视频语言", "图像文本", "跨模态")),
    ("计算机视觉", ("computer vision", "image classification", "object detection", "segmentation", "vision transformer", "visual recognition", "计算机视觉", "图像分类", "目标检测", "分割", "视觉Transformer", "视觉识别")),
    ("强化学习", ("reinforcement learning", "policy optimization", "bandit", "markov decision process", "rl", "强化学习", "策略优化", " bandit", "马尔可夫决策过程")),
    ("语音", ("speech", "asr", "automatic speech recognition", "text-to-speech", "speaker recognition", "audio", "语音", "自动语音识别", "文本到语音", "说话人识别", "音频")),
    ("推荐系统", ("recommendation", "recommender", "ctr prediction", "ranking system", "推荐系统", "点击率预测", "排序系统")),
    ("机器人", ("robot", "robotics", "manipulation", "navigation", "control policy", "机器人", "机器人学", "操作", "导航", "控制策略")),
    ("图学习", ("graph neural network", "graph learning", "molecular graph", "gnn", "图神经网络", "图学习", "分子图")),
    ("机器学习", ("machine learning", "deep learning", "neural network", "representation learning", "机器学习", "深度学习", "神经网络", "表示学习")),
    ("外泌体", ("exosome", "exosomes", "extracellular vesicles", "外泌体", "细胞外囊泡")),
    ("脊髓损伤", ("spinal cord injury", "SCI", "脊髓损伤")),
    ("压电材料", ("piezoelectric", "piezoelectricity", "PVDF", "压电", "压电材料")),
]
