#!/usr/bin/env python3
"""Shared helpers for literature reader scripts."""
...
DEFAULT_USER_AGENT = "LiteratureReader/1.0"
SHELL_CONFIG_FILES = [
    Path.home() / ".zshenv",
    Path.home() / ".zprofile",
    Path.home() / ".zshrc",
...
def runtime_config() -> dict[str, Any]:
    return {
        "obsidian_vault": env_config_value(
            "LITERATURE_READER_OBSIDIAN_VAULT",
            "READ_ARXIV_OBSIDIAN_VAULT",
        ),
        "papers_dir": env_config_value("LITERATURE_READER_PAPERS_DIR", default="Research/Papers"),
        "output_dir": env_config_value("LITERATURE_READER_OUTPUT_DIR", default="tmp/LiteratureReader"),
        "workspace_output_dir": env_config_value(
            "LITERATURE_READER_WORKSPACE_OUTPUT_DIR",
            default="LiteratureReader_output",
        ),
    }
...
def require_obsidian_vault(config: dict[str, Any]) -> Path:
    ...
    if vault_path is None:
        raise RuntimeError("Missing Obsidian vault configuration. Set LITERATURE_READER_OBSIDIAN_VAULT.")
    return vault_path
...
def resolve_note_output_mode(config: dict[str, Any]) -> tuple[str, Path]:
    ...
    output_dir = str(config.get("workspace_output_dir", "LiteratureReader_output")).strip() or "LiteratureReader_output"
...
def first_page_title_candidate(first_page_text: str) -> str:
    ...
    for i, line in enumerate(lines):
        if "论文题目（中文）" in line or "论文题目(中文)" in line:
            # 下一行开始是标题
            if i + 1 < len(lines):
                full_title = []
                # 收集连续的标题行
                for j in range(i + 1, min(i + 5, len(lines))):  # 最多收集5行
                    title_line = lines[j].strip()
                    # 停止条件：遇到空行、论文题目（外文）、作者姓名等
                    if not title_line or "论文题目" in title_line or "作者姓名" in title_line or "学科专业" in title_line or "研究方向" in title_line or "指导教师" in title_line:
                        break
                    # 跳过学校代码等无关信息
                    if "学校代码" in title_line or "分类号" in title_line or "密级" in title_line:
                        continue
                    full_title.append(title_line)
                # 如果收集到了标题行
                if full_title:
                    return clean_pdf_line(" ".join(full_title))
    
    # 查找英文标题
    for i, line in enumerate(lines):
        if "论文题目（外文）" in line or "论文题目(外文)" in line:
            # 下一行可能是标题
            if i + 1 < len(lines):
                title_line = lines[i + 1].strip()
                if title_line and len(title_line) > 5:
                    return clean_pdf_line(title_line)
    
    # 如果没有找到中文论文格式，使用原来的方法
