...
def main() -> None:
    ...
    try:
        ...
        print(f"ERROR: Failed to resolve record: {e}")
        emit({"status": "error", "error": str(e)}, args.output)
        return
    
    pdf_path = Path(str(record.get("local_pdf_path", "")).strip()).expanduser()
    ...
    if pdf_path.exists():
        try:
            ...
            print(f"WARNING: Failed to parse PDF: {exc}")
    else:
